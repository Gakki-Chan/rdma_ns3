...
namespace ns3 {
	...
	{
	        //typedef BEgressQueue<T> BEgressQueueT;
	...
	bool BEgressQueue::DoEnqueue(Ptr<Packet> p, uint32_t qIndex)
	{
		...
		{/*
                std::cout << "size of m_queues = " << m_queues.size()
                          << ", qIndex = " << qIndex << std::endl;*/
	...
	Ptr<Packet> BEgressQueue::DoDequeue(void)
	{
        ...
        //template <typename T>
	Ptr<const Packet> BEgressQueue::DoPeek(void) const	//DoPeek doesn't work for multiple queues!!
	{
		std::cout << "Warning: Call Broadcom queues without priority\n";
		NS_LOG_FUNCTION(this);
		if (m_bytesInQueueTotal == 0)
