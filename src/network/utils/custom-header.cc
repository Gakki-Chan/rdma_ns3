...
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  ...
	  }else if (l3Prot == 0xFB){ // signal TODO
		  i.WriteU32 (signal.epochID);
		  i.WriteU32 (signal.flowRate);
		  i.WriteU8 (signal.congestionPort << 1 | signal.pfcOff);
		  i.WriteU32 (signal.lastTimeStep);
	  }else if(l3Prot == 0xFA){ // polling TODO
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  signal.epochID = i.ReadU32();
		  signal.flowRate = i.ReadU32();
		  uint8_t tmp = i.ReadU8();
		  signal.congestionPort = tmp >> 1;
		  signal.pfcOff = tmp & 0x1;
		  signal.lastTimeStep = i.ReadU32();
		  l4Size = 13;
	  } else if(l3Prot == 0xFA){ // polling TODO
