...
{
  //m_fid.Print(os);
  os << " qFb=" << (unsigned) fields.m_qfb << "/" << (unsigned) fields.m_total;
}
...
void CnHeader::Serialize (Buffer::Iterator start)  const
{
  ...
  //lobyte = (lobyte & 0x00FFFFFFFFFFFFFFLLU) | (static_cast<uint64_t>(fields.m_qfb)<<56);
  //uint32_t lobyte = (m_qIndex &  0x00FFFFFFLLU) | (static_cast<uint32_t>(fields.m_qfb)<<24);
  //start.WriteU64 (hibyte);
  //start.WriteU64 (lobyte);
  start.WriteU8(m_qIndex);
  start.WriteU16(m_fid);
  start.WriteU8(m_ecnBits);
  start.WriteU16(fields.m_qfb);
  start.WriteU16(fields.m_total);
...
uint32_t CnHeader::Deserialize (Buffer::Iterator start)
{
  ...
  //fields.m_qfb = static_cast<uint8_t>(lobyte>>56);
  //m_fid.hi = hibyte;
  ...
  fields.m_qfb = start.ReadU16();
  fields.m_total = start.ReadU16();

  //fields.m_qfb = static_cast<uint8_t>(lobyte>>24);
